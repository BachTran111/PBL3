...
export async function fetchRooms(homestayId) {
  try {
    ...
    const response = await fetch(`http://localhost:8080/homestay/api/rooms/valid-homestay/${homestayId}`);
    ...
    for (const room of data) {
      ...
      const imageResponse = await fetch(`http://localhost:8080/homestay/api/rooms/${room.roomId}/images`);
      const images = await imageResponse.json();

      // Kiểm tra nếu có ảnh cho phòng
      const roomImageUrl = images.length > 0 ? images[0].imageUrl : 'assets/img/default-room.jpg'; // Dùng ảnh mặc định nếu không có ảnh
      ...
      const roomHTML = `
        ...
        <div class="room-booking">
          <button class="btn-book-now" data-room-id="${room.roomId}">Đặt ngay</button>
        </div>
      `;
